...
//            createPost(idToken);
    ...
    private void createPost(String idToken) {



        String apikey = "AIzaSyCcdllwzc0OmacJaiT5SWDNHfx88yJrcbs";
        String phone = (Singleton.instance().fetchValueString("phone"));
        String fireBaseToken = idToken;

        //Log.d("fireBaseToken", fireBaseToken);
        Api.getClient().createPost(apikey, phone, fireBaseToken).enqueue(new Callback<ResponseData>() {
            @Override
            public void onResponse(Call<ResponseData> call, Response<ResponseData> response) {

                String auth = response.body().getAuthToken();
                //Log.d("final auth", auth);
                Singleton.instance().storeValueString("authToken", auth);
                sendUserToHome();
            }
